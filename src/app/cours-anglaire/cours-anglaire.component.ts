...
@Component({
  selector: 'app-cours-anglaire',
  templateUrl: './cours-anglaire.component.html',
  styleUrls: ['./cours-anglaire.component.scss']
})
export class CoursAnglaireComponent {
  ...
  ajoutCoursAnglaire() {
    ...
    this.anglaireService.createCoursAnglaire(updateData)
    .then((res) => {
      this.loading = false;
     
      console.log('res :>> ', res);
  
      this.router.navigate(["dashboardAdmin", {index: 10}]);
    })
    .catch((err) => {
      this.loading = true;
      this.iserrorlog = true;
